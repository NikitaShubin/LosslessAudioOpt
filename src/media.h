...
// Приводит WAV к максимально совместимому виду: оставляет только канонические
// чанки fmt (16 байт, PCM) и data (PCM-байты копируются без изменений),
// посторонние чанки (fact, bext, minf, elm1, LIST и др.) удаляются. Это нужно,
// чтобы строгие кодеки (OptimFROG падает на fact, LA требует fmt первым) не
// сбоили на выходе нативных декодеров. Если заголовок уже совместим —
// возвращает true без перезаписи. Возвращает false и текст в err при ошибке
// чтения/записи; при неудаче исходный файл не изменяется.
