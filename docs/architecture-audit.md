...
- Дополнительно: секция `canonical_aliases` — синонимы внутри канонической
  схемы (нормализованное имя → канонический ключ), ранее захардкоженные
  таблицей в `canonical_key()`. Теперь добавление синонима поля (например,
  нового варианта написания «originaldate») не требует перекомпиляции.

## 1d. Осознанные исключения (не выносим из C++)

По правилам AGENTS.md всё описательное выносится в `formats/*.json`, но часть
механизмов принципиально остаётся кодом — это не хардкод «имён форматов», а
форматная/протокольная логика:

- **Бинарные парсеры и писатели контейнеров** (`tags_vorbis.cpp` — FLAC
  metadata/PICTURE/Ogg, `tags_mp4.cpp` — ilst/4CC, `tags_apev2.cpp`,
  `tags_id3.cpp`, `tags_wav.cpp`) — невозможно параметризовать через JSON;
  новый контейнер с новым типом тегов всегда потребует парсер в C++.
- **Сниффинг контейнера в `extract_tags`** (magic `fLaC`/`OggS`/`ID3`/
  `APETAGEX` и подстроки `matroska`/`webm`/`mp4`/`wav`/`riff` в имени ffprobe)
  — это определение фактического контейнера произвольного входного файла, а
  не выбор формата из конфига. Оставлено в C++ как изначальный слой
  диспетчеризации; новые форматы с уже известными контейнерами покрываются
  существующими сигнатурами.
- **Глобальные конвенции ключей**: префикс `replaygain_`
  (`is_replaygain`), префиксы ffmpeg-кодеков `pcm_`/`dsd_` в
  `media::codec_is_lossless` — устойчивые соглашения, не формат-специфичны,
  перенос в JSON не даёт расширяемости.
- **Дефолт формата восстановления** (`serve_entry.cpp`: `restore_to="flac"`)
  и text дефолтов в справке/веб-UI — осознанный выбор команды, не таблица
  форматов; валидируется против `formats/*.json` при старте.
- Прочие литералы (`engine.kind=="binary"|"ffmpeg"`, `verify.kind`,
  `download.kind`) — значения полей конфига, их разбор в C++ корректен по
  data-driven-модели.
